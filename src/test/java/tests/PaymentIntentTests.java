...
public class PaymentIntentTests extends BaseClass {
    ...
    public void cleanup() {
        ...
        // Clean up payment methods FIRST, while their customer still exists
        logger.info("🧹 Cleaning up {} created payment methods...", paymethodIds.size());
        for (String id : paymethodIds) {
            try {
                Response getResp = endpoints.paymentMethods.retrievePaymentMethod(id);
                String attachedCustomer = getResp.jsonPath().getString("customer");

                if (attachedCustomer != null) {
                    endpoints.paymentMethods.detachPaymentMethod(id);
                    logger.info("🧹 Detached payment method: {}", id);
                } else {
                    logger.info("ℹ️ Payment method {} was never attached, skipping detach", id);
                }
            } catch (Exception e) {
        ...
        // Clean up customers AFTER payment methods are handled
